...
% change this value at LGM and at Present
T_bottom= 0;
T_up =10;


f4=figure(4); axis square; box on; hold on
P1=histogram(Tbase_final,n_Tbase,'FaceColor',[0.2 0.2 0.2]);
[hi cx]=hist(Tbase_final,n_Tbase);
AA=median(Tbase_final);
P2=bar(AA,max(hi));
P3=bar(T_bottom,max(hi));
legend([P1, P2, P3], 'Histogram','Predicted median', 'Actual','Location', 'NorthEast');
legend boxoff
xlabel('Temp at 20 ka, T_{base} (^oC)');
ylabel('Frequency');
xlim([-20 30+T_up]);
set(gca,'FontSize',20);
ax.LineWidth = 2.0;
...
P1=histogram(Tsurf_final,n_Tsurf,'FaceColor',[0.2 0.2 0.2]);
[hi cx]=hist(Tsurf_final,n_Tsurf);
BB=median(Tsurf_final);
P2=bar(BB,max(hi));
P3=bar(T_up,max(hi));
legend([P1, P2, P3], 'Histogram','Predicted median', 'Actual','Location', 'NorthWest');
legend boxoff;
xlabel('Temp at present, T_{present} (^oC)');
ylabel('Frequency');
xlim([-20 30+T_up]);
set(gca,'FontSize',20);
ax.LineWidth = 2.0;
...
% print(f4,'MBTP9_Path1_Tbase', '-dpdf', '-r300');
% print(f5,'MBTP9_Path1_Tamp', '-dpdf', '-r300');
% print(f6,'MBTP9_Path1_Tamp_vs_Tbase', '-dpdf', '-r300');
